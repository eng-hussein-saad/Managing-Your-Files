...
describeDatabase("administrator retained audit history", () => {
  ...
  it("projects live, deleted, and system actors with allowlisted metadata and no read audit", async () => {
    ...
    await prisma.auditLog.createMany({ data: [
      { actorId: administratorUserId, action: "auth.login", entityType: "USER", entityId: administratorUserId, metadata: { outcome: "SUCCESS", password: "must-not-project" }, createdAt: now },
