...
describeDatabase("administrator permanent user deletion", () => {
  ...
  it("removes objects and dependent rows, nulls the actor, and audits once", async () => {
    ...
    await prisma.auditLog.create({ data: { actorId: primaryUserId, action: "auth.login", entityType: "USER", entityId: primaryUserId, metadata: { outcome: "SUCCESS" }, createdAt: new Date() } });
    ...
    const formerActor = await prisma.auditLog.findFirst({ where: { action: "auth.login" } });
