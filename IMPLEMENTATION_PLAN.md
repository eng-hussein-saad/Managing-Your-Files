...
Establish the application foundation and complete the identity/security layer so all later features can rely on a stable authentication, authorization, database, API, and frontend shell.
...
Axios and React Query should have a reusable authentication strategy rather than handling tokens separately in each screen.
...
- A new user can register, receive an OTP, verify their email, log in, access protected user pages, retrieve their profile, refresh an expired access session, and log out.
...
- Authentication state survives normal page navigation and refresh-token behavior is verified.
...
Build the platform foundation and complete identity system for Managing Your Files. Users must be able to register, verify their email using an expiring OTP, resend verification codes, log in, maintain an authenticated session, view their profile, and log out. The system has regular users and administrators, and protected functionality must enforce authentication and roles securely. Establish the shared application foundation required by later file-management functionality while keeping concerns cleanly separated and maintainable.

### `/speckit.plan` constraints

Use the required project stack: Next.js App Router, TypeScript, Tailwind CSS, Framer Motion, TanStack React Query, Axios, Express.js, TypeScript, Prisma ORM, PostgreSQL, JWT authentication, and a monorepo with `client/` and `server/`. Include refresh-token authentication, email OTP delivery, backend input validation, secure password hashing, admin bootstrap through environment variables, consistent error handling, and an audit logging foundation. Design the Prisma schema with later file, folder, soft-delete, and audit requirements in mind.
