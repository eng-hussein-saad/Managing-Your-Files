...
For Phase 2 add the ten server-only settings: `UPLOAD_MAX_FILE_SIZE_BYTES=5242880`, `USER_STORAGE_QUOTA_BYTES=104857600`, `UPLOAD_ALLOWED_MIME_TYPES`, `UPLOAD_MAX_FILES_PER_BATCH=10`, `SUPABASE_URL`, secret `SUPABASE_SECRET_KEY`, private `SUPABASE_STORAGE_BUCKET`, `FILE_QUERY_DEFAULT_PAGE_SIZE`, `FILE_QUERY_MAX_PAGE_SIZE`, and `FILE_EXTRACTION_MAX_BYTES`. Do not expose the secret or any file-management setting through `NEXT_PUBLIC_*` variables. Vercel hosts the client only; Render stores all ten settings as server environment variables. This repository has no Dockerfile, so no Docker configuration needs synchronization.
