...
- [X] T003 [P] Add and validate all eight Phase 2 server configuration keys, fixed limits, MIME allowlist, secret classification, and safe examples in server/src/config/env.ts, server/.env.example, and server/tests/unit/config.test.ts; constrain file page sizes to the UI options 5, 10, and 20
- [X] T004 [P] Document the eight Phase 2 settings and their Vercel/Render deployment mappings without exposing the Supabase secret, document client-selected file page sizes, and record the current absence or required synchronization disposition of Docker/deployment configuration in docs/deployment.md
