...
4. **Given** an owned file, **When** the user downloads it, **Then** the complete content is returned with the original filename and correct content metadata without creating an audit event for the read operation.
...
5. **Given** file-object or metadata removal cannot be completed, **When** deletion is attempted, **Then** the operation is not reported as fully successful, quota is not overstated as reclaimed, a safe retryable outcome is provided, and reasonable best-effort compensation or sanitized operational logging is used without exposing unauthorized file content.
...
- Audit recording fails during an important state change; the primary operation follows the documented fail-open behavior, the audit failure is logged without sensitive data, and no cross-system rollback or durable reconciliation mechanism is required.
...
- **FR-007**: A successful upload MUST result in accessible stored content and an active metadata record containing owner, optional folder, original display name, storage reference, verified content type, size, and lifecycle timestamps. When storage and metadata outcomes differ, the system MUST return a safe per-file outcome, MUST NOT expose partial content through the application, and MUST attempt reasonable best-effort compensation with sanitized operational logging if compensation does not complete.
...
- **FR-022**: File deletion MUST require explicit irreversible-action confirmation and, when fully successful, MUST permanently remove both the owned file's metadata record and corresponding Supabase Storage object. It MUST attempt to emit a sanitized audit event. Available user quota MUST increase exactly once when the authoritative metadata record is permanently removed, and audit-write failure MUST NOT require reversing an otherwise successful deletion.
...
- **FR-025**: File deletion MUST be idempotent and MUST NOT be reported as fully successful when a known metadata or stored-object removal step fails. Partial provider or persistence failures MUST produce a safe retryable outcome, MUST NOT expose unauthorized file content, MUST NOT reclaim quota more than once, and MUST use reasonable best-effort compensation or sanitized operational logging. Temporary private inaccessible objects or temporarily unavailable metadata MAY remain after an infrastructure failure; Phase 2 does not require a durable outbox, storage-operation table, reconciliation worker, or distributed transaction.
...
- **FR-029**: Successful uploads, file moves, file deletions, folder creations, folder renames, and folder deletions MUST attempt to produce audit events through the shared audit capability with actor, action, target, timestamp, and sanitized useful metadata. Reads such as preview and download, failed requests, validation failures, and denied operations MUST NOT produce audit events.
- **FR-030**: Audit events and audit-failure operational logs MUST NOT contain file content, extracted text, submitted path fragments, storage credentials, internal storage locations, authentication secrets, or more filename information than operationally necessary.
- **FR-031**: Audit persistence failure MAY allow the primary mutation to continue and SHOULD produce sanitized operational logging. Audit behavior MUST NOT require an atomic transaction between persistence and file storage. Preview and download authorization MUST expose no content when denied and MUST NOT create audit records when successful.
...
- **FR-037**: Each user MUST be limited to 100 MB of currently retained active metadata-backed file content, defined as 104,857,600 bytes. The quota MUST count each owned active file record's authoritative size exactly once and MUST exclude failed uploads, successfully deleted records, and private inaccessible provider objects left by partial infrastructure failure.
...
| FR-022–FR-025 | User Story 5 scenarios plus irreversible confirmation, cancel, repeated deletion, object/metadata outcomes, best-effort compensation, empty-folder enforcement, audit-attempt behavior, and exact quota-reclamation checks |
...
| FR-028–FR-031 | All user stories plus keyboard/accessibility review, audit-attempt completeness when available, fail-open audit behavior, sanitized operational logging, and prohibited-data scans |
...
- **SC-001**: At least 9 of 10 first-time test participants can upload a valid three-file batch through either drag-and-drop or the picker and correctly identify each file's progress and final outcome without assistance in under 2 minutes, using the usability verification protocol below.
...
- **SC-003**: In the performance verification protocol below, at least 95 of 100 measured successful file-collection requests over 10,000 owned records MUST return the first page within 2 seconds, and unchanged ordered results MUST contain no duplicates across consecutive pages.
...
- **SC-005**: At least 9 of 10 first-time test participants can locate a designated known file by search or type/folder filter, open its details, and initiate preview or download in no more than 5 user actions from My Files, using the usability verification protocol below.
...
- **SC-010**: When audit persistence is available, 100% of successful Phase 2 operations covered by the audit policy produce structurally complete audit events. When audit persistence is deliberately unavailable, 100% of tested primary operations follow the documented fail-open behavior and produce sanitized operational logging, with zero prohibited secrets, file content, extracted text, or storage locations found in audit and output scans.
- **SC-011**: At least 9 of 10 first-time test participants correctly identify the cause and available next action for upload rejection, no-results, unsupported-preview, non-empty-folder deletion, and retryable failure states without assistance, using the usability verification protocol below.
...
- **Performance protocol for SC-003**: Run the production-built API on Linux with Node.js 24 using at least 2 dedicated CPU cores and 4 GB RAM, with PostgreSQL on the same host and no competing workload. Seed exactly 10,000 deterministic FILE rows for the measured owner plus 1,000 foreign-owner rows, distributed across every supported type category, root and ten owned folders, varied names, sizes, and upload dates. After one warm-up for each query template, issue 100 measured requests at concurrency 5: 20 repetitions each of (Q1) default newest-first page 1, (Q2) a case-insensitive filename search matching at least 100 rows, (Q3) type plus root-folder filter on page 2 sorted by name ascending, (Q4) type plus owned-folder filter on page 5 sorted by size descending, and (Q5) combined filename, type, and owned-folder filters on page 3 sorted by upload date ascending. Measure server wall-clock time from receipt of the authenticated request through completion of the JSON response. Report p50, p95, maximum, failures, deterministic fixture seed, per-template results, database version, hardware, and production-build commit. A failed request does not count as satisfying the two-second target.
- **Usability protocol for SC-001, SC-005, and SC-011**: Collect exactly 10 valid sessions from participants who have not used or been shown the Phase 2 file-management interface. Give every participant the same neutral written task script, seeded dataset, device class, starting route, and permitted browser capabilities without coaching or interface hints. Record completion time, user-action count, assistance, final outcome, and each state-interpretation answer using a scoring key written before the sessions. A participant passes an SC only by meeting every condition stated by that SC without assistance. Report anonymized per-participant results, aggregate pass counts, the script, scoring key, browser viewport, and any excluded session with its reason; every excluded session MUST be replaced so the final evidence contains exactly 10 valid sessions.
...
- Phase 2 uses the constitution's default fail-open audit behavior. Important successful operations attempt audit recording, but audit persistence failure may allow the primary operation to continue with sanitized operational logging. Cross-system file operations use reasonable best-effort compensation and do not require a durable outbox, storage-operation table, reconciliation worker, or distributed transaction.
...
- Full Docker support, deployment hardening, the complete cross-phase automated test suite, and final developer documentation beyond Phase 2 configuration changes (Phase 4). Phase 2 still synchronizes every configuration artifact required by the constitution, including any existing Docker/deployment configuration, and records an explicit not-applicable result when no such artifact exists. Supabase Storage provider selection and integration are Phase 2 responsibilities, not a deferred Phase 4 migration.
