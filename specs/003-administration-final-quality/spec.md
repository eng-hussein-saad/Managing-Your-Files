...
- **FR-022**: Successful important file/folder mutations and administrator-controlled resource mutations MUST attempt to emit centralized sanitized audit events. This includes upload, file move/deletion, folder creation/rename/deletion, administrator bootstrap, user role change, permanent user deletion, and permanent administrator file deletion. Authentication, authorization denials, preview/download, and administrator list, detail, statistics, and audit-history reads MUST NOT create audit events. Audit-write failure MUST remain fail-open for the primary mutation and MUST produce sanitized operational evidence.
...
- **Audit Event**: A retained, sanitized record of an important successful file/folder or administrator-controlled resource mutation with actor when available, action, target, timestamp, and safe metadata; after the actor account is removed, it uses a generic "Deleted user" presentation without retaining the actor's name or email snapshot.
