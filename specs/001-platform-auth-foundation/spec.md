...
2. **Given** registration, verification, sign-in, logout, credential rotation, or denied role access, **When** the operation completes, **Then** no audit event is created because ordinary authentication and authorization operations are outside the audit policy.
...
- A soft-deleted user presents correct credentials, an unexpired access token, or an otherwise valid refresh token; sign-in, protected access, and renewal all fail without disclosing the account's deletion status.
...
- **FR-010**: Only a verified, non-deleted account with valid credentials MUST be able to sign in, and failed sign-in responses MUST avoid revealing whether the account is absent, deleted, unverified, or has an incorrect password. Phase 1 MUST NOT throttle failed sign-in attempts or lock accounts because of repeated sign-in failures.
...
- **FR-023**: Every protected service operation MUST validate the access credential and confirm that its user is not soft-deleted server-side before returning protected data or performing a protected action.
...
- **FR-026**: The Phase 1 data contract MUST establish users, verification codes, refresh-token records, file records, nested folders, and audit events with ownership and lifecycle relationships sufficient for later phases without exposing later file-management behavior now. The nullable user deletion timestamp MUST be the only soft-delete marker: files and folders MUST NOT have soft-delete fields. It MUST NOT introduce a separate refresh-session entity.
- **FR-027**: Registration, verification, sign-in, logout, renewal rotation, and authorization-denial outcomes MUST NOT create audit records. Audit history is reserved for successful important file/folder mutations and administrator-controlled resource mutations.
...
- **User**: A person's account and identity status, including normalized email, display name, non-recoverable password representation, verification state, role (`USER` or `ADMIN`), nullable soft-deletion timestamp, other lifecycle timestamps, and relationships to refresh tokens, owned folders, owned files, and attributable audit events. A soft-deleted user cannot authenticate or use protected operations.
...
- **File**: The Phase 1 ownership and lifecycle contract for a future uploaded item, including owner, optional folder, name and content metadata, storage reference, nullable extracted content, and lifecycle timestamps. File deletion is permanent and deferred to Phase 2; files have no soft-delete marker.
- **Folder**: The Phase 1 ownership and hierarchy contract for future organization, including owner, name, optional parent folder, and lifecycle timestamps. Folder deletion is permanent and deferred to Phase 2; folders have no soft-delete marker.
...
- Soft deletion applies only to users. User deletion behavior is delivered in Phase 3; file and folder deletion is permanent in Phase 2.
