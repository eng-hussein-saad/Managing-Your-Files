...
- **FR-040**: All redesigned surfaces MUST conform to WCAG 2.2 Level AA for applicable semantics, names, labels, keyboard operation, focus behavior, contrast, reflow, status communication, and pointer-target requirements. Normal text MUST provide at least 4.5:1 contrast, large text and meaningful non-text user-interface graphics/boundaries MUST provide at least 3:1 contrast, and status MUST NOT rely on color alone. Every author-controlled standalone interactive target MUST provide a hit area of at least 44 by 44 CSS pixels; inline links in flowing text and unmodified user-agent controls MAY use the applicable WCAG 2.2 SC 2.5.8 exception but MUST still satisfy its 24 CSS-pixel size-or-spacing rule.
...
- **SC-001**: 100% of the surface rows and every applicable default, loading, empty, validation, submitting/disabled, success, failure, retry, access-denied, and confirmation state defined by `contracts/ui-acceptance.md` pass side-by-side acceptance review against the approved source or closest approved pattern at 1440 px, 768 px, and 390 px in both light and dark appearance, plus the required 320 px overflow/action review; every reviewed combination MUST have a recorded pass, approved deviation, or blocking result, and every material visual or responsive deviation MUST receive explicit project-maintainer approval before acceptance.
...
- **SC-005**: All major workflows can be completed using only a keyboard and meet the applicable WCAG 2.2 Level AA rules made normative by FR-040. Automated axe checks MUST report zero critical or serious violations on the landing, authentication, dashboard, files, profile, and administrator route families; every other reported WCAG A/AA finding MUST be remediated or recorded with reproducible evidence that it is a false positive, and manual evidence MUST verify contrast, 44 by 44 CSS-pixel project targets and permitted SC 2.5.8 exceptions, focus behavior, 200% zoom/reflow, and keyboard operation that automation cannot establish.
