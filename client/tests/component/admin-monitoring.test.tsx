...
import { adminAuditResponseFixture, adminStatisticsResponseFixture } from "../fixtures/admin";

const useAdminStatistics = vi.fn();
const useAdminAuditEvents = vi.fn();
vi.mock("../../src/features/admin/hooks/use-admin-monitoring", () => ({
  useAdminStatistics: /** Supplies deterministic administrator statistics. */ () => useAdminStatistics(),
  useAdminAuditEvents: /** Supplies deterministic sanitized audit history. */ (...args: unknown[]) => useAdminAuditEvents(...args),
}));

describe("administrator monitoring experience", () => {
  it("renders exact totals and safe recent uploads", () => {
    useAdminStatistics.mockReturnValue({ isLoading: false, error: null, data: adminStatisticsResponseFixture() });
  ...
  it("renders live, deleted, and system actor labels without unsafe metadata", () => {
    ...
    useAdminAuditEvents.mockReturnValue({ isLoading: false, error: null, data: { data: [base, { ...base, id: "60000000-0000-4000-8000-000000000060", actor: { kind: "deleted", label: "Deleted user" } }, { ...base, id: "60000000-0000-4000-8000-000000000061", actor: { kind: "system", label: "System" } }], meta: { totalPages: 1 } } });
    render(<AdminAuditHistory query={{ direction: "desc", page: 1, pageSize: 20 }} update={vi.fn()} />);
