...
const deletion = { mutateAsync: vi.fn(), reset: vi.fn(), isPending: false, error: null };
vi.mock("../../src/features/admin/hooks/use-admin-files", () => ({
  useAdminFiles: /** Supplies deterministic global file query state. */ (...args: unknown[]) => useAdminFiles(...args),
  useDeleteAdminFile: /** Supplies deterministic global deletion state. */ () => deletion,
}));
const query = { sort: "uploadedAt" as const, direction: "desc" as const, folder: "any" as const, page: 1, pageSize: 20 as const };
...
describe("administrator global file experience", () => {
  ...
  it("covers loading and responsive metadata results", () => {
    ...
    expect(screen.getByRole("status")).toHaveTextContent("Loading global file metadata");
    view.unmount();
    useAdminFiles.mockReturnValueOnce({ isLoading: false, error: null, data: { data: [adminFileResponseFixture()], meta: { totalPages: 1 } }, refetch: vi.fn() });
    render(<ToastProvider><AdminFileDirectory query={query} update={vi.fn()} /></ToastProvider>);
  ...
  it("requires the exact filename and identifies the owner", () => {
    ...
    render(<ToastProvider><AdminFileActions file={file} onStale={vi.fn()} /></ToastProvider>);
    fireEvent.click(screen.getByRole("button", { name: "Delete permanently" }));
    expect(screen.getByText(new RegExp(file.owner.email))).toBeInTheDocument();
    expect(screen.getByRole("textbox", { name: new RegExp(file.originalName) })).toBeInTheDocument();
  });
});
