...
const changeRole = { mutateAsync: vi.fn(), reset: vi.fn(), isPending: false, error: null };
const deleteUser = { mutateAsync: vi.fn(), reset: vi.fn(), isPending: false, error: null };
vi.mock("../../src/features/admin/hooks/use-admin-users", () => ({
  useAdminUsers: /** Supplies deterministic administrator user query state. */ (...args: unknown[]) => useAdminUsers(...args),
  useChangeAdminUserRole: /** Supplies the deterministic role mutation. */ () => changeRole,
  useDeleteAdminUser: /** Supplies the deterministic deletion mutation. */ () => deleteUser,
}));

const query = { sort: "createdAt" as const, direction: "desc" as const, page: 1, pageSize: 20 };
...
describe("administrator user experience", () => {
  ...
  it("covers loading, empty, and populated URL-state controls", () => {
    ...
    view.unmount();
    useAdminUsers.mockReturnValueOnce({ isLoading: false, error: null, data: { data: [], meta: { totalPages: 0 } } });
  ...
  it("opens role and typed-email permanent deletion confirmations", () => {
    ...
    const view = render(<ToastProvider><AdminUserActions user={user} onStale={vi.fn()} /></ToastProvider>);
    fireEvent.click(screen.getByRole("button", { name: "Change role" }));
    expect(screen.getByRole("dialog")).toHaveAccessibleName(`Change ${user.name}'s role?`);
